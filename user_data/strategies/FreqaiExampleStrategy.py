...
class FreqaiExampleStrategy(IStrategy):
    ...
    exit_prediction_threshold = 0.001
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        target_mean = df["&-s_close_mean"]
        target_std = df["&-s_close_std"]

        enter_long_conditions = [
            df["do_predict"] == 1,
            df["&-s_close"] > self.entry_prediction_threshold,
            df["&-s_close"] > target_mean + self.entry_zscore_threshold * target_std,
        ]
    ...
    def populate_exit_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        exit_long_conditions = [
            df["do_predict"] == 1,
            (
                (df["&-s_close"] < self.exit_prediction_threshold)
                | (df["&-s_close"] < df["&-s_close_mean"])
            ),
        ]
